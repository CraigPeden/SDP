...
from ..Shared.Funcs import simpleAngle
...
def _buildPacket(type, one, two, three):
    ...
    timestamp = time.time()
    try:
        c = struct.pack('cHHddf', type, one, two, timestamp, timestamp, simpleAngle(three))
    except struct.error:
...
class VisionServer(LineReceiver):
    """ Serves vision data """
    def __init__(self, simulation, fps=25):
    ...
    def connectionMade(self):
        ...
        print "VISION: Got client"
        self.state = "GETREQUEST"
        self.clock = pygame.time.Clock()

    def connectionLost(self, reason):
        """ Runs when a client disconnects """
        print "VISION: Lost client: %s" % reason
        self.state = "GETCLIENT"
...
class VisionServerFactory(Factory):
    ...
    def buildProtocol(self, addr):
        return VisionServer(self.s, self.fps)

class VisionServerStarter:
    """ Runs vision as a subprocess """
    def __init__(self, simulation, fps=25):
        self.s = simulation
        self.vsf = VisionServerFactory(simulation, fps)
    ...
    def _run(self, port):
        ...
        try:
            print "VISION: Listening on port %d" % port
            reactor.run()
