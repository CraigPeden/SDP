...
class DefenderIntercept:
	...
	def pick_action(self):
		...
		if self.ball.velocity > -0.5:
			...
			if self.ball.y > self.world.our_goal[0].y and self.ball.y < self.world.our_goal[1].y:
				y=self.ball.y
			else:
				top_diff =  self.ball.y -  self.world.our_goal[1].y
				bottom_diff = 	self.world.our_goal[0].y - self.ball.y

				if top_diff > bottom_diff:
					y = self.world.our_goal[0].y
				else:
					y = self.world.our_goal[1].y
