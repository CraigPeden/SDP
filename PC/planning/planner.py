...
class Planner:
	...
	def __init__(self, our_side, pitch_num, our_color, our_role):
		...
		self._world.our_defender.catcher_area = {'width' : 35, 'height' : 30, 'front_offset' : 12} #10
		self._world.our_attacker.catcher_area = {'width' : 35, 'height' : 25, 'front_offset' : 14}
	...
	def plan(self):
		...
		if self.robot_role == 'defender':
			...
			if self._world.pitch.zones[our_attacker.zone].isInside(ball.x, ball.y):
				if self._world.our_attacker.has_ball(ball):
					print 'AttackerShoot'
					print our_attacker.catcher
					self._world.our_attacker.catcher_area = {'width' : 50, 'height' : 50, 'front_offset' : 5}
